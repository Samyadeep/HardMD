...
def get_sets(args):
    if args.dataset == 'cifar_fs':
        ...
        if args.eval:
            # Trainset and Valset is set to None
...
def get_loaders(args, num_tasks, global_rank):
    # datasets
    if args.eval:
        print("######### Loader for evaluation .... ##########")
        # This will get kicked on during testing --- args.eval == True (Pure Testing)
        _, _, dataset_vals = get_sets(args)
    else:
        # This will get kicked on during training --- args.eval == False (Validation during Training)
