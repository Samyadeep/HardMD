...
class ProtoNet_Finetune(ProtoNet):
    ...
    def forward(self, supp_x, supp_y, x):
        ...
        params = []
        adaptation_params = [v for k,v in self.backbone.named_parameters() if v.requires_grad == True]
        params.append({'params': adaptation_params})
        
        #print("Number of parameter blocks: {}".format(len(adapter_params)))
        # create optimizer
        opt = torch.optim.Adam(self.backbone.parameters(),
                               lr=self.lr,
                               betas=(0.9, 0.999),
        ...
        # opt = torch.optim.Adadelta(params,
        #                        lr=self.lr,
        #                        weight_decay=0.)
        ...
        def single_step(z, mode=True):
            ...
            with torch.set_grad_enabled(mode):
                ...
                # Integrating lite framework --- for memory efficient training
                #with torch.no_grad():
        ...
        #t2 = time.time() 
        #print("Time Difference: {}".format(diff_))
...
class ProtoNet_AdaTok(ProtoNet):
    ...
    def forward(self, supp_x, supp_y, x):
        ...
        print(" ################ Time for only optimizing the model ############### : {}".format(t2-t1))
